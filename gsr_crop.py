...
def mfcc(signal, samplerate=512, winlen=2, winstep=0.5, numcep=13,
         nfilt=26, nfft=None, lowfreq=0, highfreq=None, preemph=0.97, ceplifter=22, appendEnergy=True,
         winfunc=lambda x: numpy.ones((x,))):
    """Compute MFCC features from a bio signal.

    :param signal: the signal from which to compute features. Should be an N*1 array
    :param samplerate: the sample rate of the signal we are working with, in Hz.
    :param winlen: the length of the analysis window in seconds. Default is 2s
    :param winstep: the step between successive windows in seconds. Default is 0.5s
...
def fbank(signal, samplerate=512, winlen=2, winstep=0.5,
...
def get_filterbanks(nfilt=20, nfft=512, samplerate=512, lowfreq=0, highfreq=None):
...
def process_gsr_with_mfcc(csv_path, output_path):
    signal = []
...
def main(gsr_folder_path, output_folder_path):
    ...
    for subject in os.listdir(gsr_folder_path):
            ...
            if os.path.isdir(subject_path):
                ...
                logging.info(f"Processed {subject} gsr")
            else:logging.error(f"Subject is not a directory: {subject_path}")
...
if __name__ == '__main__':
    gsr_folder = r""
    output_folder = r""
    if gsr_folder and output_folder:
        main(gsr_folder, output_folder)
    else:
        print("Please set gsr_folder and output_folder paths")